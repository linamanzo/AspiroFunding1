    ...
    {public DB_Personnal_Details_DAO()
        {
            ...
            cmdGetBy = "SELECT * FROM personnaldetails WHERE @id = id";
            cmdGetScore = "SELECT personnalscore FROM personnal WHERE @id = idpersd";
            cmdInsert = String.Empty;
            
        }
        protected override void ObjectToParameters(Borrower obj, NpgsqlCommand command)
        {
